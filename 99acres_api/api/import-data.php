    ...
    if ($_POST['option'] == 'Buy') {
        ...
        if (count($data) > 1) {
            for ($i = 1; $i < sizeof($data); $i++) {
                ...
                // $tupleNew__priceValWrap = str_replace('₹', '', $tupleNew__priceValWrap);
                // $tupleNew__perSqftWrap = str_replace('₹', '', $tupleNew__perSqftWrap);
                ...
                $tupleNew__area2Type_2 = str_replace(['(', ')'], '', $tupleNew__area2Type_2);
        ...
        $headerRow = $data[0] ?? array(); // Assuming the first row contains headers
        if (!in_array('tupleNew__perSqftWrap', $headerRow)) {
            // Handle the case where the column header doesn't exist
            echo json_encode(["error" => "Column 'tupleNew__perSqftWrap' does not exist in the CSV file."]);
            exit;
        }
        //$res1=$db->delete('inventory', 'property_type','');
        ...
        if (count($data) > 1) {
            for ($i = 1; $i < sizeof($data); $i++) {
                ...
                $tupleNew__perSqftWrap = isset($data[$i][array_search('tupleNew__perSqftWrap', $headerRow)]) ? trim($data[$i][array_search('tupleNew__perSqftWrap', $headerRow)]) : 'N/A';
                // Remove ₹ sign if present
                // $tupleNew__priceValWrap = str_replace('₹', '', $tupleNew__priceValWrap);
                // $tupleNew__perSqftWrap = str_replace('₹', '', $tupleNew__perSqftWrap);
                ...
                $tupleNew__possessionBy = ""; //not available in rent excell
