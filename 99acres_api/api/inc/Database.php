...
class Database
{
    ...
    {
        try {
$this->pdo = new PDO("mysql:host=" . $hostname . ";dbname=" . $db_name . ";port=" . $port_number, $username_db, $password_db,array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
     ...
     */
    public function insert($table, $dat, $mode = 'STRICT_TRANS_TABLES')
    {
        ...
        $im  = implode(', ', $mark);
        $ins = $this->pdo->prepare("INSERT $mode INTO $table ($col) values ($im)");
        try {
            $ins->execute($data);
            return true;
        }
